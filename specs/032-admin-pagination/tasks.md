...
- [X] T037 Merge & post-merge regression (Constitution XII + XIII gate 3): merge `story/032-admin-pagination` → `master`, `git checkout master`, rebuild, restart the Host, re-run the Playwright admin suite (05, 07, 10, 11, 16) on merged code, capture passing output; session ends on `master`

...
- [ ] T004 [US2] `.github/workflows/ci.yml` per ADR 0011: triggers
...
- [ ] T005 [US2] Workflow steps, in order: checkout@v4 → setup-dotnet@v4
...
- [ ] T006 [US2] Validate: YAML parses (parser, not eyeballs); every
...
- [ ] T007 [US3] Local command sequence, in workflow order, with env
...
- [ ] T008 [US3] Independent verification (Constitution XVI): fresh
...
- [ ] T009 [US3] Gate 3 (post-merge, on master): rebuild, restart,
      re-run the unit sequence + Playwright; mark all tasks `[X]`, set
      spec Status to Complete, commit F
